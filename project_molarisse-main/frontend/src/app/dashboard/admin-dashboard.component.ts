import { Component, OnInit, HostListener, ElementRef, ViewChild, CUSTOM_ELEMENTS_SCHEMA } from '@angular/core';
...
export class AdminDashboardComponent implements OnInit {
    ...
    this.authService.getCurrentUser().subscribe({
      next: (user) => {
        ...
        if (user && user.id) {
          ...
          // Set profile image URL directly
          this.profileImageUrl = `${this.apiUrl}/api/v1/api/users/profile/picture-by-id/${user.id}?t=${timestamp}`;
          console.log('Set profile image URL:', this.profileImageUrl);
        } else {
  ...
  getAdminProfilePicture(): string {
    if (this.currentUser && this.currentUser.id) {
      try {
        ...
        const timestamp = new Date().getTime();
        const url = `${this.apiUrl}/api/v1/api/users/profile/picture-by-id/${this.currentUser.id}?t=${timestamp}`;
        console.log('Using direct profile picture URL by ID:', url);
        return url;
      } catch (error) {
  ...
  // Getter for profile image URL - similar to doctor dashboard
  get profileImage(): string {
    if (!this.profileImageUrl || this.profileImageUrl.includes('default-avatar')) {
      // If we don't have a profile image URL, try to generate one from the user ID
      if (this.currentUser?.id) {
        const timestamp = new Date().getTime();
        return `${this.apiUrl}/api/v1/api/users/profile/picture-by-id/${this.currentUser.id}?t=${timestamp}`;
      }
      return '/assets/images/default-avatar.png';
    }
