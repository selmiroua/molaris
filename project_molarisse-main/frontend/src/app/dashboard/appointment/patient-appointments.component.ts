...
export class PatientAppointmentsComponent implements OnInit {
  ...
  getFilteredAppointments(): Appointment[] {
    ...
    } else if (this.activeTab === 'annulés' || this.activeTab === 'cancelled') {
      // Cancelled or rejected appointments
      filtered = filtered.filter(a => 
        a.status === AppointmentStatus.CANCELED || a.status === AppointmentStatus.REJECTED
      );
    } else if (this.activeTab === 'terminés' || this.activeTab === 'completed') {
  ...
  onPageSizeChange(value: number): void {
    ...
    // You may want to reload appointments from server with new pagination
    // this.loadAppointments();
  }
    ...
    try {
      ...
      if (parentElement) {
        ...
        if (appointmentCard) {
          ...
          const doctorNameElement = appointmentCard.querySelector('.doctor-info h3');
