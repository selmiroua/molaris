...
export class MedicalCheckupStepperComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // Check for existing ordonnances for all teeth
    if (this.fichePatient?.patientId) {
      this.allTeeth.forEach(tooth => {
        this.ordonnanceService.existsForTooth(this.fichePatient.patientId, tooth.num).subscribe(exists => {
          console.log(`[ngOnInit] (2) Checking ordonnance for patientId=${this.fichePatient.patientId}, toothNum=${tooth.num} => exists:`, exists);
          this.ordonnanceExistsForTooth[tooth.num] = exists;
          if (exists) {
            this.toothApproval[tooth.num] = {
              status: 'approved',
  ...
  setToothApproval(toothNum: number, status: 'approved' | 'not_approved') {
    ...
    this.ordonnanceService.existsForTooth(this.fichePatient.patientId, toothNum).subscribe(exists => {
      ...
      // If not exists, proceed as before
      if (status === 'approved') {
  ...
  ordonnanceExistsForTooth: { [toothNum: number]: boolean } = {};
  ...
  viewOrdonnanceForTooth(toothNum: number) {
    ...
    this.ordonnanceService.getOrdonnanceForTooth(this.fichePatient.patientId, toothNum).subscribe(ordonnances => {
      ...
      if (ordonnances && ordonnances.length > 0) {
        ...
        const formattedDate = new Date(ordonnanceToView.date);
        ...
        this.dialog.open(OrdonnanceDetailsDialog, {
          data: { 
            ordonnance: { 
              ...ordonnanceToView,
              date: formattedDate,
              // If signature is a placeholder, show a message
              signature: ordonnanceToView.signature === '[SIGNATURE]' ? 'Signature non disponible' : ordonnanceToView.signature
            } 
          },
          height: '500px',
          width: '500px',
