...
export class AppointmentService {
    ...
    return this.getDoctorAppointments(doctorId).pipe(
      map(appointments => {
        ...
        const appointmentsOnDate = appointments.filter(apt => {
          ...
          return aptDate === selectedDate && isActive;
        });
  ...
  getUserProfilePicture(userId: number): Observable<string | null> {
    ...
    return this.http.get<any>(`${environment.apiUrl}/api/v1/api/users/profile/picture/${userId}`, { headers })
      .pipe(
        map(user => {
          console.log('User data received:', user);
          // The user object contains the profilePicturePath field
          // We just return this path - the component will construct the full URL
          if (user && user.profilePicturePath) {
            console.log('Profile picture path found:', user.profilePicturePath);
