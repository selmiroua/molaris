...
export class OrdonnanceFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // PATCH: If readonly ordonnance, patch form with ordonnance data
    if (this.data?.ordonnance) {
      ...
      // Set all controls to readonly (disable the form)
  ...
  save() {
    if (this.ordonnanceForm.valid) {
      ...
      const payload = {
        ...
        toothNumber: Number(this.data?.toothNumber) // Ensure toothNumber is a number
      };
    ...
    <div class="ordonnance-card-preview">
      ...
      <div class="ordonnance-footer-block">
        <div class="signature-section">
          ...
          <ng-container *ngIf="isSignatureImage(ordonnance.signature); else noSignature">
            <img [src]="ordonnance.signature" alt="Signature" class="signature-image" />
          </ng-container>
          <ng-template #noSignature>
            <div class="no-signature-message">{{ ordonnance.signature }}</div>
          </ng-template>
          <div class="tunis-date">
            Tunis, le {{ ordonnance.date | date:'d MMMM yyyy':'':'fr' }}
          </div>
        </div>
      </div>
