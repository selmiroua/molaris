...
export class UserSearchComponent implements OnInit, OnChanges {
    ...
    this.http.get<any[]>(`${environment.apiUrl}/api/v1/api/users/admins`, { headers: this.getHeaders() }).pipe(
      ...
      next: (admins) => {
        if (admins && admins.length > 0) {
          ...
          this.filteredUsers = [...this.admins];
  ...
  filterUsers(): void {
    const query = this.internalSearchQuery?.toLowerCase().trim() || '';
  ...
  getProfileImageUrl(user: User): string {
    ...
    // Use the user ID directly for the picture-by-id endpoint
    return `${environment.apiUrl}/api/v1/api/users/profile/picture-by-id/${user.id}?t=${new Date().getTime()}`;
  }
