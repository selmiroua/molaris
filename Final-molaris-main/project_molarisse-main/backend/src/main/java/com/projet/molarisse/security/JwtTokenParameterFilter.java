 ...
 * Specifically designed for document and media endpoints where tokens are passed via query params
 */
@Component
...
public class JwtTokenParameterFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(
        ...
        // Process requests for document endpoints and message media endpoints
        String servletPath = request.getServletPath();
        boolean isDocumentPath = servletPath.startsWith("/api/users/documents/");
        boolean isMediaPath = servletPath.contains("/api/messages/media/") || 
                              servletPath.contains("/api/v1/api/messages/media/");
        
        // Log all paths for debugging
        logger.debug("JwtTokenParameterFilter processing path: {}", servletPath);
        
        // If the path is not for documents or media, continue to the next filter
        if (!isDocumentPath && !isMediaPath) {
        ...
        if (token == null || token.isEmpty()) {
            logger.debug("No token found in URL parameter for path: {}", servletPath);
            filterChain.doFilter(request, response);
            return;
        }
