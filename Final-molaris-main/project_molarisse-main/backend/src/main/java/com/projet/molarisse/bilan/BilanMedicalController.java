...
public class BilanMedicalController {
    ...
    @GetMapping("/me")
    public ResponseEntity<List<BilanMedical>> getMyBilans(Authentication authentication) {
        try {
            // Get current user (doctor)
            User currentUser = (User) authentication.getPrincipal();
            
            // Get all bilans for this doctor
            List<BilanMedical> doctorBilans = bilanMedicalService.getBilansForDoctor(currentUser.getId());
            
            return ResponseEntity.ok(doctorBilans);
        } catch (Exception e) {
            logger.error("Error retrieving bilans for current doctor: {}", e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
        }
    }
} 