    ...
    <div class="message-bell">
      ...
      <mat-menu #messageMenu="matMenu" class="message-menu">
        <div class="message-header">
          <h3>Messages</h3>
          <button mat-button (click)="navigateToMessaging()">
        ...
        <div class="message-list">
          ...
          <div *ngIf="!loading && conversations.length === 0" class="no-messages">
            <p>Aucun message</p>
            <button mat-button color="primary" (click)="navigateToMessaging()">
...
export class MessageBellComponent implements OnInit {
  ...
  openConversation(conversation: any): void {
    if (conversation && conversation.partnerId) {
      // First navigate to appropriate dashboard with messaging section
      this.navigateToMessaging();
      
      // Then use a timeout to allow the messaging component to initialize before navigating to the specific conversation
      setTimeout(() => {
        this.router.navigate(['/dashboard/messaging', conversation.partnerId]);
      }, 100);
    } else {
      this.navigateToMessaging();
    }
  ...
  navigateToMessaging(): void {
    ...
    switch (this.userRole.toLowerCase()) {
      case 'doctor':
        ...
        break;
      case 'secretary':
        dashboardRoute = '/dashboard/secretary';
        break;
      case 'admin':
    ...
    // Navigate to the dashboard with messaging section active
    this.router.navigate([dashboardRoute], { queryParams: { section: 'messaging' } });
  }
  ...
  formatDate(date: any): string {
    ...
    try {
      ...
      const messageDateOnly = new Date(dateObj.getFullYear(), dateObj.getMonth(), dateObj.getDate());
