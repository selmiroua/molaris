...
export class AuthGuard implements CanActivate, CanActivateChild {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): boolean {
    ...
    const isAuthCheck = route.data['authCheck'] === true;

    // Special handling for the auth-check route (wildcard redirects here)
    if (isAuthCheck) {
    ...
    // Remove the banned check redirection
    // We'll just let banned users access their dashboard with disabled components
