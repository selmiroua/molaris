...
@Component({
  ...
  host: {
    ...
    '[class.editing-patient-info]': 'isEditingPatientInfo'
  },
  imports: [
    CommonModule,
    MatDialogModule,
  ...
  template: `
    <div class="appointment-flex-container">
      <div class="medical-stepper-panel" *ngIf="showMedicalStepper">
        <app-medical-checkup-stepper [open]="showMedicalStepper" [fichePatient]="fichePatient" (cancel)="showMedicalStepper = false" (saveButtonClicked)="handleSaveButtonClicked($event)" [highlightSaveButton]="isSaveButtonHighlighted"></app-medical-checkup-stepper>
      </div>
            ...
            <div class="main-panel" [class.hidden-panel]="activeTab !== 'main'">
              ...
              <div class="panel-content">
                ...
                <div *ngIf="!isLoading" class="content-wrapper">
                  ...
                  <div class="payment-section">
                    <div class="payment-row">
                      <div class="payment-info">
                        ...
                        <span class="bill-number">facture #{{ getBillNumber() || 'N/A' }}</span>
                        <span *ngIf="remainingToPay !== null && remainingToPay <= 0" class="payment-status paid-status" style="background:#e6f9ed;color:#22c55e;padding:4px 12px;border-radius:8px;font-weight:600;">Payée</span>
                        <span *ngIf="remainingToPay === null || remainingToPay > 0" class="payment-status unpaid-status" style="background:#fde8e8;color:#ef4444;padding:4px 12px;border-radius:8px;font-weight:600;">Impayé</span>
                  ...
                  <div class="patient-info-grid" [formGroup]="patientInfoForm" *ngIf="isEditingPatientInfo; else viewPatientInfo">
                    <div class="info-field">
                    ...
                    <div class="info-field">
                      ...
                      <div class="field-content">
                        ...
                        <mat-form-field appearance="outline" class="full-width-field">
                          <input matInput formControlName="dateNaissance" [matDatepicker]="picker" placeholder="Date de naissance">
                    ...
                    <div class="info-field full-width">
                      ...
                      <div class="field-content">
                        ...
                        <mat-form-field appearance="outline" class="full-width-field">
                          <textarea matInput formControlName="adresse" placeholder="Adresse" rows="2"></textarea>
                        </mat-form-field>
                      </div>
                    </div>
                  </div>
                  
                  <ng-template #viewPatientInfo>
                  ...
                  <div class="medical-buttons-simple">
                    ...
                    <button class="add-record-btn-simple" (click)="addMedicalRecord()">
                      <mat-icon>{{ isEditingPatientInfo ? 'save' : 'description' }}</mat-icon>
                      <span class="btn-text-simple">{{ isEditingPatientInfo ? 'Enregistrer' : 'Modifier Fiche Patient' }}</span>
                    </button>
                  </div>
  ...
  styles: [`
    ...
    .editing-patient-info .patient-info-grid mat-form-field {
      margin-bottom: 8px;
    }
...
export class AppointmentDetailDialogComponent implements OnInit {
  ...
  bilanMedical: any = null;
  timeline: any[] = [];
  bilanMedicalForm: any = { valid: true }; // Placeholder for form validation
  medicalFileForm: any = { valid: true }; // Placeholder for form validation
  ...
  handleSaveButtonClicked(paymentData: any): void {
    this.isSaveButtonHighlighted = true;
    
    // If we have payment data, just store it locally for later use when "Terminer" is clicked
    if (paymentData) {
      // Update local component properties with the received payment data
      ...
      this.paymentNotes = paymentData.paymentNotes;
      this.remainingToPay = paymentData.remainingToPay;
      
      // Show success message for bilan save only
      this.snackBar.open('Bilan médical enregistré avec succès', 'Fermer', {
        duration: 3000,
  ...
  // Calculate profit based on the payment amount with a fee deduction
  calculateProfit(): number {
    return (this.amountPaid || 0) * 0.984; // Example: 1.6% fee
  }
  
  // Calculate remaining amount to be paid
  calculateRemainingAmount(): number {
    return Math.max(0, (this.amountToPay || 0) - (this.amountPaid || 0));
  }
  ...
  // Add a property to track edit mode
  isEditingPatientInfo = false;
  patientInfoForm!: FormGroup;
  
  // Initialize the patient info form
  initPatientInfoForm() {
    this.patientInfoForm = this.fb.group({
      telephone: [this.fichePatient?.telephone || ''],
      email: [this.appointment.patient?.email || ''],
      dateNaissance: [this.fichePatient?.dateNaissance || ''],
      sexe: [this.fichePatient?.sexe || ''],
      adresse: [this.fichePatient?.adresse || '']
    });
  }
  
  addMedicalRecord() {
    // Toggle edit mode for patient info
    this.isEditingPatientInfo = !this.isEditingPatientInfo;
    
    if (this.isEditingPatientInfo) {
      // Initialize the form with current values
      this.initPatientInfoForm();
    } else if (this.patientInfoForm?.valid) {
  ...
  savePatientInfo() {
    if (!this.patientInfoForm.valid) return;
    
    const formValues = this.patientInfoForm.value;
    
    // Update local fichePatient object
    if (this.fichePatient) {
      this.fichePatient.telephone = formValues.telephone;
      this.fichePatient.dateNaissance = formValues.dateNaissance;
      this.fichePatient.sexe = formValues.sexe;
      this.fichePatient.adresse = formValues.adresse;
    }
    
    // Update patient email if changed
    if (this.appointment.patient && formValues.email !== this.appointment.patient.email) {
      this.appointment.patient.email = formValues.email;
    }
    
    // Call API to update patient info using the /{patientId}/fiche endpoint
    if (this.fichePatient && this.appointment.patient?.id) {
      this.patientService.updateFicheById(this.appointment.patient.id, this.fichePatient).subscribe({
        next: (result) => {
          this.fichePatient = result;
          this.isEditingPatientInfo = false;
          this.snackBar.open('Informations du patient mises à jour avec succès', 'Fermer', { duration: 3000 });
        },
        error: (err) => {
          console.error('Error updating patient info:', err);
          // Keep edit mode on if there's an error
          this.snackBar.open('Erreur lors de la mise à jour des informations du patient', 'Fermer', { duration: 3000 });
        }
      });
    } else {
      // If no patient ID is available, just toggle edit mode off and show local update message
      this.isEditingPatientInfo = false;
      this.snackBar.open('Informations du patient mises à jour localement', 'Fermer', { duration: 3000 });
    }
  }
  ...
  finishTreatment() {
    ...
    // First save payment data to consultation
    if (this.appointment) {
      // Get payment data from the component properties
      const paymentData = {
        ...
        remainingToPay: this.calculateRemainingAmount(),
        // Include bilanMedicalId if available
        bilanMedicalId: this.bilanMedical?.id || 0
      };
      
      console.log('Finishing treatment with payment data:', paymentData);
      
      // Save payment data to consultation
      this.bilanMedicalService.savePaymentToConsultation(this.appointment.id, paymentData).subscribe({
        next: (paymentResult) => {
          console.log('Payment data saved to consultation successfully:', paymentResult);
          
          // Then update appointment status to COMPLETED
          this.statusForm.patchValue({ status: AppointmentStatus.COMPLETED });
          this.updateStatus();
          
          this.snackBar.open('Traitement terminé avec succès', 'Fermer', {
            duration: 3000,
            panelClass: ['success-snackbar']
          });
        },
        error: (paymentErr) => {
          console.error('Error saving payment data to consultation:', paymentErr);
          this.snackBar.open('Erreur lors de l\'enregistrement du paiement', 'Fermer', {
            duration: 3000,
      ...
      this.snackBar.open('Aucun rendez-vous sélectionné', 'Fermer', { duration: 3000 });
    }
  }

  canFinish(): boolean {
    // Check if both medical checkup and record exist
    return this.bilanMedicalForm.valid && this.medicalFileForm.valid;
  }
}