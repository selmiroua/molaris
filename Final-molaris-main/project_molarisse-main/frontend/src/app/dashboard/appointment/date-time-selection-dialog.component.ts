  ...
  loadDoctorId(): void {
    this.profileService.getCurrentProfile().subscribe({
      next: (profile) => {
        ...
        // Approach using 'any' to avoid TypeScript errors
        ...
        if (profileAny.role) {
          if (typeof profileAny.role === 'string') {
            isDoctor = profileAny.role.toLowerCase() === 'doctor';
          } else if (profileAny.role.name) {
            isDoctor = profileAny.role.name === 'DOCTOR';
          } else {
            isDoctor = profileAny.role === 'DOCTOR';
          }
        }
        
        if (isDoctor) {
          this.doctorId = profileAny.id ?? null;
          console.log('Connected doctor, using own ID:', this.doctorId);
        ...
        // Otherwise, fallback to secretary logic
        ...
        if (secretaryProfile && secretaryProfile.assignedDoctor && secretaryProfile.assignedDoctor.id) {
          ...
          console.log('Found assigned doctor ID:', this.doctorId);
          if (this.dateTimeForm.get('date')?.valid) {
            this.onDateSelected();
          }
        } else {
          // Fallback: if the user has an id, use it (for doctors)
          if (profileAny.id) {
            this.doctorId = profileAny.id;
            console.log('Fallback: using own ID as doctor:', this.doctorId);
            if (this.dateTimeForm.get('date')?.valid) {
              this.onDateSelected();
            }
          } else {
