import { Component, Input, OnInit, ViewChild, ElementRef, Output, EventEmitter, ChangeDetectorRef, Inject, Pipe, PipeTransform } from '@angular/core';
...
import { OrdonnanceFormComponent, OrdonnanceDetailsDialog } from '../../ordonnance-form/ordonnance-form.component';
import { MatListModule } from '@angular/material/list';
import { TimeAgoPipe } from '../shared/pipes/file-size.pipe';
...
@Component({
  selector: 'app-ordonnance-confirmation-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
  ...
  template: `
    <div class="dialog-container">
      ...
      <div mat-dialog-content class="dialog-content">
        ...
        <p class="question">Voulez-vous créer une ordonnance pour cette dent?</p>
      </div>
      
      <div mat-dialog-actions class="dialog-actions">
        <button mat-button class="btn-no" (click)="dialogRef.close('no')">Non, approuver uniquement</button>
        <button mat-button class="btn-yes" (click)="dialogRef.close('yes')">Oui, créer une ordonnance</button>
      </div>
    </div>
  `,
  styles: [`
...
export class OrdonnanceConfirmationDialog {
  constructor(
    public dialogRef: MatDialogRef<OrdonnanceConfirmationDialog>,
...
@Component({
  ...
  template: `
    <div class="main-panels-wrapper">
      <div class="stepper-panel-modern">
        <div class="stepper-title-fixed">
          <h1 class="stepper-main-title">{{ currentStep === 3 ? 'Paiement' : 'Bilan Médical' }}</h1>
        </div>
          ...
          <div *ngFor="let step of steps; let i = index" class="step-modern-fr" [class.active]="i === currentStep" [class.completed]="i < currentStep">
            <div class="step-icon-modern-fr" [ngClass]="{'active-step': i === currentStep}">
              <ng-container *ngIf="i < currentStep">
                ...
                <svg *ngIf="i === 3" width="32" height="32" viewBox="0 0 32 32"><g stroke="#22c55e" stroke-width="2.2" fill="none"><circle cx="16" cy="16" r="10"/><path d="M12 16h8"/><path d="M16 12v8"/></g></svg>
              </ng-container>
              <ng-container *ngIf="i === currentStep">
                ...
                <svg *ngIf="i === 3" width="32" height="32" viewBox="0 0 32 32"><g stroke="#2563eb" stroke-width="2.2" fill="none"><circle cx="16" cy="16" r="10"/><path d="M12 16h8"/><path d="M16 12v8"/></g></svg>
              </ng-container>
              <ng-container *ngIf="i > currentStep">
                ...
                <svg *ngIf="i === 3" width="32" height="32" viewBox="0 0 32 32"><g stroke="#555" stroke-width="2.2" fill="none"><circle cx="16" cy="16" r="10"/><path d="M12 16h8"/><path d="M16 12v8"/></g></svg>
              </ng-container>
            </div>
        ...
        <div class="stepper-content-card">
          ...
          <ng-container *ngIf="currentStep === 1">
            <ng-container *ngIf="currentSubStep2 === 1">
              ...
              <div class="tooth-chart-svg-wrapper" style="display: flex; justify-content: center; align-items: flex-start; margin-top: -30px; margin-bottom: 0; position: relative;">
                ...
                <svg viewBox="0 0 520 550" width="380" height="450" style="display:block;">
                    ...
                    <g *ngFor="let tooth of upperTeeth; let i = index">
                      <g [attr.transform]="getToothTransformArch(i, 'upper')">
                        <path [attr.d]="getToothPathByType(tooth.type)"
                              ...
                              (click)="onValidationToothClick(tooth.num, getToothArchPosition(i, 'upper').x, getToothArchPosition(i, 'upper').y)"
                              (mouseenter)="onToothMouseEnter(tooth.num, getToothArchPosition(i, 'upper').x, getToothArchPosition(i, 'upper').y)"
                    ...
                    <g *ngFor="let tooth of lowerTeeth; let i = index">
                      <g [attr.transform]="getToothTransformArch(i, 'lower')">
                        <path [attr.d]="getToothPathByType(tooth.type)"
                              [attr.fill]="getToothFill(tooth.num)"
                              stroke="#666" stroke-width="1" class="tooth-shape"
                              (click)="onValidationToothClick(tooth.num, getToothArchPosition(i, 'lower').x, getToothArchPosition(i, 'lower').y)"
                              (mouseenter)="onToothMouseEnter(tooth.num, getToothArchPosition(i, 'lower').x, getToothArchPosition(i, 'lower').y)"
                              (mouseleave)="onToothMouseLeave()" />
                        <path [attr.d]="getToothGrooveByType(tooth.type)" stroke="#999" stroke-width="0.6" fill="none" />
                      </g>
                  ...
                  <g transform="translate(-40, 0)">
                    <rect x="210" y="540" width="18" height="18" rx="3" fill="#0047AB" />
                    <text x="235" y="554" font-size="14" fill="#666">Déjà traité</text>
                    <rect x="340" y="540" width="18" height="18" rx="3" fill="#FF0000" />
                ...
                <div *ngIf="showCosmeticForm" class="cosmetic-modal-overlay">
                  <div class="cosmetic-modal-content">
                    ...
                    <ng-container *ngIf="showCosmeticForm">
                      <div *ngIf="selectedCosmeticTooth !== null" class="popup-compact popup-shadow popup-rounded cosmetic-popup-modern" [ngStyle]="{ left: cosmeticPopupX + 'px', top: cosmeticPopupY + 'px', position: 'absolute' }">
              ...
              <div *ngFor="let tooth of coloredTeeth" class="tooth-approval-card-v2">
                <div class="tooth-approval-header">
                  ...
                  <div class="tooth-approval-condition-row">
                    ...
                    <div class="tooth-approval-actions-inline" style="flex-direction: column; align-items: flex-end;">
                      <ng-container *ngIf="ordonnanceExistsForTooth[tooth.num]">
                        <button mat-icon-button color="primary" style="margin-bottom: 6px;" (click)="viewOrdonnanceForTooth(tooth.num)">
                      ...
                      <div style="display: flex; flex-direction: row; gap: 8px;">
                        <button type="button"
                          [ngClass]="{
                            'approval-btn-v3': true,
                            'approved-btn-v3': toothApproval[tooth.num]?.status === 'approved' || ordonnanceExistsForTooth[tooth.num],
                            'selected': toothApproval[tooth.num]?.status === 'approved' || ordonnanceExistsForTooth[tooth.num]
                          }"
                          [disabled]="ordonnanceExistsForTooth[tooth.num]"
                          (click)="setToothApproval(tooth.num, 'approved')">
                        ...
                        <button type="button"
                          [ngClass]="{
                            'approval-btn-v3': true,
                            'not-approved-btn-v3': toothApproval[tooth.num]?.status === 'not_approved',
                            'selected': toothApproval[tooth.num]?.status === 'not_approved'
                          }"
                          [disabled]="ordonnanceExistsForTooth[tooth.num]"
                          (click)="setToothApproval(tooth.num, 'not_approved')">
                          <mat-icon>schedule</mat-icon>
                          <mat-icon style="font-size:16px;margin-left:-10px;position:relative;top:2px;color:#ef4444;">close</mat-icon>
                          Pas maintenant
                        </button>
                      </div>
                    </div>
        ...
        <ng-container *ngIf="currentStep === 3">
          ...
          <div class="payment-section">
            <div class="payment-section-header">
              ...
              <h3 class="payment-section-title">Détails du paiement</h3>
            </div>
            ...
            <form [formGroup]="paymentForm" class="payment-form-row">
              ...
              <mat-form-field appearance="outline" class="payment-field reste-field" floatLabel="always">
                ...
                <input matInput [value]="remainingToPay | number:'1.2-2'" readonly>
                <span matPrefix>€&nbsp;</span>
                <mat-icon matSuffix [ngClass]="{'paid-icon': remainingToPay <= 0, 'unpaid-icon': remainingToPay > 0}">
                  {{ remainingToPay <= 0 ? 'check_circle' : 'account_balance_wallet' }}
                </mat-icon>
              </mat-form-field>
            </form>
          ...
          <div class="notes-section">
            ...
            <mat-form-field appearance="outline" class="notes-field">
              <mat-label>Notes sur la consultation </mat-label>
              <textarea matInput formControlName="paymentNotes" rows="4" placeholder="Ajouter des notes sur le paiement ou le traitement..."></textarea>
            </mat-form-field>
          </div>
          
          
          
          <!-- Payment step has no action buttons, using the footer buttons instead -->
        </ng-container>
    ...
    <div *ngIf="hoveredTooth !== null && toothData[hoveredTooth]" class="tooth-info-card" [ngStyle]="{ left: hoverX + 'px', top: hoverY + 'px' }"
      ...
      <button class="edit-info-btn" (click)="editToothFromInfoCard()">Edit</button>
    </div>
    <div *ngIf="selectedTooth !== null && (currentStep === 1 || currentStep === 2)" class="popup-compact popup-shadow popup-rounded" [ngStyle]="{ left: popupX + 'px', top: popupY + 'px' }">
      ...
      <mat-form-field appearance="outline" class="popup-form-field">
        <mat-select [(ngModel)]="toothPopup.condition" placeholder=" condition">
          <mat-select-trigger>
      ...
      <mat-form-field *ngIf="toothPopup.condition === 'autre'" appearance="outline" class="popup-form-field">
        <input matInput [(ngModel)]="toothPopup.customCondition" placeholder="Précisez la condition..." />
      </mat-form-field>
      <mat-form-field appearance="outline" class="popup-form-field">
        <mat-select [(ngModel)]="toothPopup.treatment" placeholder=" traitement">
          <mat-option *ngFor="let treatment of treatments" [value]="treatment.value">
            <span [ngStyle]="{'background-color': getTreatmentColor(treatment.value)}">{{ treatment.label }}</span>
          </mat-option>
        </mat-select>
      </mat-form-field>
      <mat-form-field *ngIf="toothPopup.treatment === 'autre'" appearance="outline" class="popup-form-field">
      ...
      <div class="popup-btn-row">
        <button *ngIf="!toothApproval[selectedTooth]?.status && currentStep === 2"
                class="popup-btn approval-btn-v3" 
                (click)="setToothApproval(selectedTooth, 'approved')">
          Approuver
        </button>
        <button *ngIf="toothApproval[selectedTooth]?.status === 'approved' && currentStep === 2"
  ...
  `,
  styles: [`
    /* Ordonnance Confirmation Dialog Styles */
    .ordonnance-confirmation-overlay {
      position: fixed;
      top: 0;
      left: 0;
      right: 0;
    ...
    .ordonnance-confirmation-header {
      padding: 16px 20px;
      background-color: #2563eb;
      color: white;
    }
    
    .ordonnance-confirmation-header h3 {
      margin: 0;
      font-size: 18px;
      font-weight: 600;
    }
    
    .ordonnance-confirmation-body {
      padding: 20px;
      text-align: center;
    }
    
    .ordonnance-confirmation-body p {
      font-size: 16px;
      margin-bottom: 16px;
      color: #333;
    }
    ...
    .ordonnance-confirmation-actions {
      display: flex;
      padding: 16px 20px;
    ...
    .bilan-documents-section h4:after {
      content: '';
      position: absolute;
      bottom: 0;
      left: 0;
...
export class MedicalCheckupStepperComponent implements OnInit {
  ...
  @Input() highlightSaveButton = false;
  @Output() bilanSaved = new EventEmitter<any>();
  paymentForm!: FormGroup;
  steps = [
    { icon: 'chat', label: 'Médical' },
    { icon: 'event', label: 'Traitement' },
    { icon: 'medical_services', label: 'Validation' },
    { icon: 'payments', label: 'Paiement' }
  ];
  currentStep = 0;
  currentSubStep2 = 1;
  ...
  hoverX: number = 0;
  hoverY: number = 0;
  keepTooltipVisible: boolean = false;
  ...
  constructor(private fb: FormBuilder, private bilanMedicalService: BilanMedicalService, private patientService: PatientService, private dialog: MatDialog, private http: HttpClient, private snackBar: MatSnackBar, private sanitizer: DomSanitizer, private ordonnanceService: OrdonnanceService, private cdr: ChangeDetectorRef) {
  ...
  ngOnInit(): void {
            ...
            if (data) {
              ...
              if (data.toothData) {
                try {
                  ...
                  // Initialize toothApproval for all teeth with treatments
                  Object.keys(this.toothData).forEach(toothNum => {
                    const num = Number(toothNum);
                    // Always initialize tooth approval for teeth with treatments
                    this.toothApproval[num] = this.toothApproval[num] || {
                      status: 'not_approved',  // Default to not approved for new teeth
                      reason: '',
                      otherReason: ''
                    };
                  });
                  console.log('Initialized toothApproval:', this.toothApproval);
                } catch (e) {
    ...
    // Check for ordonnance existence for each tooth
    if (this.fichePatient && this.fichePatient.patientId) {
      // First check ordonnance existence for teeth with treatments
      Object.keys(this.toothData).forEach(toothNumStr => {
        const toothNum = Number(toothNumStr);
        this.ordonnanceService.existsForTooth(this.fichePatient.patientId, toothNum).subscribe(exists => {
          console.log(`[ngOnInit] Checking ordonnance for patientId=${this.fichePatient.patientId}, toothNum=${toothNum} => exists:`, exists);
          this.ordonnanceExistsForTooth[toothNum] = exists;
          if (exists) {
            this.toothApproval[toothNum] = {
              status: 'approved',
      ...
      // Then check for all other teeth
      this.allTeeth.forEach(tooth => {
        if (!this.toothData[tooth.num]) { // Only check teeth not already checked above
          this.ordonnanceService.existsForTooth(this.fichePatient.patientId, tooth.num).subscribe(exists => {
            console.log(`[ngOnInit] Checking ordonnance for patientId=${this.fichePatient.patientId}, toothNum=${tooth.num} => exists:`, exists);
            this.ordonnanceExistsForTooth[tooth.num] = exists;
            if (exists) {
              this.toothApproval[tooth.num] = {
                status: 'approved',
                ordonnanceCreated: true
              };
            }
  ...
  openToothPopup(tooth: number, x: number, y: number) {
    ...
    // Default offsets
    let popupX = x + 60;
    let popupY = (tooth >= 31 && tooth <= 48) ? y - 180 : y + 60;
    // Popup size (approximate)
    const popupWidth = 320;
    const popupHeight = 220;
    const margin = 16;
    // Clamp X so popup stays within viewport
    const maxX = window.innerWidth - popupWidth - margin;
    const minX = margin;
    if (popupX > maxX) popupX = maxX;
    if (popupX < minX) popupX = minX;
    // Clamp Y so popup stays within viewport (and above SVG)
    const maxY = window.innerHeight - popupHeight - margin;
    const minY = margin;
    if (popupY > maxY) popupY = maxY;
    if (popupY < minY) popupY = minY;
    this.popupX = popupX;
    this.popupY = popupY;
  }
  deleteToothPopup() {
    this.selectedTooth = null;
    this.toothPopup = { condition: '', treatment: '', note: '', otherCondition: '', otherTreatment: '', otherTreatmentType: 'single', customCondition: '' };
  }
  saveToothPopup() {
    if (this.selectedTooth !== null) {
      // If condition is 'autre', save the customCondition value to otherCondition
      if (this.toothPopup.condition === 'autre' && this.toothPopup.customCondition) {
        this.toothPopup.otherCondition = this.toothPopup.customCondition;
      }
      
      // If treatment is 'autre', save the otherTreatment value
      if (this.toothPopup.treatment === 'autre' && this.toothPopup.otherTreatment) {
        // Keep the original treatment as 'autre' but save the custom text
        console.log('Saving custom treatment:', this.toothPopup.otherTreatment);
      }
      ...
      this.toothData[this.selectedTooth] = {
        ...
        otherTreatmentType: 'single' // Remove multi/single distinction as requested
      };
  ...
  getConditionLabel(conditionValue: string): string {
    if (!conditionValue) return 'Aucune condition';
    
    // Check if it's a custom 'autre' condition
    if (conditionValue === 'autre') {
      // For tooth info card or other cases where we need to look up by tooth number
      const toothNum = this.hoveredTooth !== null ? this.hoveredTooth : this.selectedTooth;
      if (toothNum !== null && this.toothData[toothNum]?.otherCondition) {
        return `Autre: ${this.toothData[toothNum].otherCondition}`;
      }
      return 'Autre condition';
    }
    
    // Return French label for standard conditions
    switch(conditionValue) {
      case 'carie': return 'Carie';
      case 'fracture': return 'Fracture';
      case 'abces': return 'Abcès';
      case 'sensibilite': return 'Sensibilité';
      case 'gingivite': return 'Gingivite';
      default: return conditionValue; // Fallback to the value itself
    }
  }
  ...
  hideToothInfoTimeout: any = null;
  
  hideToothInfo() {
    // Clear any existing timeout
    if (this.hideToothInfoTimeout) {
      clearTimeout(this.hideToothInfoTimeout);
    }
    
    // Set a new timeout to hide the tooltip after a delay
    this.hideToothInfoTimeout = setTimeout(() => {
      if (!this.keepTooltipVisible) {
        this.hoveredTooth = null;
      }
    }, 500); // 500ms delay gives time to move to the tooltip
  }
  getSelectedTreatmentLabel(treatmentValue: string): string {
    if (!treatmentValue) return 'Aucun traitement';
    
    // Check if it's a custom 'autre' treatment
    if (treatmentValue === 'autre') {
      // For tooth info card or other cases where we need to look up by tooth number
      const toothNum = this.hoveredTooth !== null ? this.hoveredTooth : this.selectedTooth;
      if (toothNum !== null && this.toothData[toothNum]?.otherTreatment) {
        return `Autre: ${this.toothData[toothNum].otherTreatment}`;
      }
      return 'Autre traitement';
    }
    
    // Return French label for standard treatments
    switch(treatmentValue) {
      case 'blanchiment': return 'Blanchiment';
      case 'detartrage': return 'Détartrage';
      case 'plombage': return 'Plombage';
      case 'extraction': return 'Extraction';
      case 'couronne': return 'Couronne';
      default: return treatmentValue; // Fallback to the value itself
    }
  }

  editToothFromInfoCard() {
    if (this.hoveredTooth !== null) {
      // Get the tooth position for popup placement
      let position;
      const toothNum = this.hoveredTooth;
      
      // Find the tooth in upper or lower teeth arrays
      const upperIndex = this.upperTeeth.findIndex(t => t.num === toothNum);
      const lowerIndex = this.lowerTeeth.findIndex(t => t.num === toothNum);
      
      if (upperIndex !== -1) {
        position = this.getToothArchPosition(upperIndex, 'upper');
      } else if (lowerIndex !== -1) {
        position = this.getToothArchPosition(lowerIndex, 'lower');
      } else {
        // Fallback to current hover position
        position = { x: this.hoverX, y: this.hoverY };
      }
      
      // Open the tooth popup at the tooth position
      this.onValidationToothClick(toothNum, position.x, position.y);
    }
  }
  ...
  saveAllBilan() {
    ...
    const payload: any = {
      ...
      // Payment fields removed from bilan_medical payload
    };
    ...
    this.bilanMedicalService.saveBilanMedical(payload, this.uploadedFiles).subscribe({
      next: (data) => {
        ...
        // Emit event to parent component to save payment data to consultation
        // The parent component will handle saving payment data to the consultation
        this.saveButtonClicked.emit(paymentData);
        ...
        if (this.fichePatient && this.fichePatient.id && this.fichePatient.patientId) {
          ...
          this.patientService.createOrUpdateFiche(this.fichePatient.patientId, updatedFichePatient).subscribe({
            ...
            error: (updateErr) => {
              ...
              alert('Bilan médical enregistré .');
              this.currentStep = 0;
            }
          });
  ...
  isColoredTooth(toothNum: number): boolean {
    ...
    // A tooth is colored if it has data, is selected, or has an approval status
    const hasToothData = !!this.toothData[toothNum];
    const isSelected = this.selectedTeeth.includes(toothNum);
    const hasApprovalStatus = !!this.toothApproval[toothNum]?.status;
    const hasOrdonnance = !!this.ordonnanceExistsForTooth[toothNum];
    
    // Log for debugging
    if (hasToothData || isSelected || hasApprovalStatus || hasOrdonnance) {
      console.log(`Tooth ${toothNum} is colored: hasToothData=${hasToothData}, isSelected=${isSelected}, hasApprovalStatus=${hasApprovalStatus}, hasOrdonnance=${hasOrdonnance}`);
    }
    
    return hasToothData || isSelected || hasApprovalStatus || hasOrdonnance;
  }
  ...
  onCosmeticToothClick(toothNum: number, x: number, y: number) {
    if (this.isColoredTooth(toothNum)) {
      ...
      // Find the SVG and modal elements
      ...
      if (svg && modal) {
        ...
        if (ctm) {
          ...
          const popupHeight = 220;
          // Position relative to modal: right of tooth, vertically centered
          let left = screenPt.x - modalRect.left + 32;
          let top = screenPt.y - modalRect.top - popupHeight / 2;
          // Clamp right edge
          if (left + popupWidth > modalRect.width) {
            left = modalRect.width - popupWidth - 8;
          }
          // Clamp left edge
          if (left < 8) {
            left = 8;
          ...
          // Clamp bottom edge
          if (top + popupHeight > modalRect.height) {
            top = modalRect.height - popupHeight - 8;
          }
          // Clamp top edge
          if (top < 8) {
            top = 8;
  ...
  ordonnanceExistsForTooth: { [key: number]: boolean } = {};
  toothApproval: { [toothNum: number]: { status: 'approved' | 'not_approved', reason?: string, otherReason?: string, ordonnanceCreated?: boolean } } = {};
  ...
  // Always check with backend before proceeding
  this.ordonnanceService.existsForTooth(this.fichePatient.patientId, toothNum).subscribe(exists => {
    if (exists) {
      this.ordonnanceExistsForTooth[toothNum] = true;
      this.toothApproval[toothNum] = {
        status: 'approved',
        ordonnanceCreated: true
      };
    ...
    // If not exists, proceed with new confirmation dialog
    if (status === 'approved') {
      // Show confirmation dialog first
      this.showOrdonnanceConfirmation(toothNum);
    } else {
      this.toothApproval[toothNum] = {
        status: 'not_approved',
        ordonnanceCreated: false
      };
      this.toothApproval = { ...this.toothApproval };
      this.cdr.markForCheck();
    }
  });
}

// Show a confirmation dialog asking if the doctor wants to create an ordonnance
showOrdonnanceConfirmation(toothNum: number) {
  // Create a simple confirmation dialog
  const dialogRef = this.dialog.open(OrdonnanceConfirmationDialog, {
    width: '450px',
    data: { toothNum: toothNum, toothName: this.toothNames[toothNum] || '' }
  });

  dialogRef.afterClosed().subscribe(result => {
    if (result === 'yes') {
      // Open ordonnance form
      const ordonnanceDialogRef = this.dialog.open(OrdonnanceFormComponent, {
        width: '700px',
        data: {
          patient: {
            ...this.fichePatient,
            id: this.fichePatient?.patientId // Ensure patient.id is set!
          },
          doctor: this.fichePatient?.doctor && this.fichePatient.doctor.id
            ? this.fichePatient.doctor
            : { ...this.fichePatient.doctor, id: this.fichePatient?.doctorId },
          toothNumber: toothNum
        }
      });

      ordonnanceDialogRef.afterClosed().subscribe(ordonnanceResult => {
        if (ordonnanceResult === true) {
          this.toothApproval[toothNum] = {
            status: 'approved',
            ordonnanceCreated: true
          };
      ...
      // Just approve without ordonnance
      this.toothApproval[toothNum] = {
        status: 'approved',
        ordonnanceCreated: false
      };
    ...
    // If dialog was dismissed without selection, do nothing
  });
}
  ...
  get coloredTeeth() {
    // Make sure all teeth with treatments are included
    const teethWithTreatments = Object.keys(this.toothData).map(Number);
    return this.allTeeth.filter(t => this.isColoredTooth(t.num) || teethWithTreatments.includes(t.num));
  }
  ...
  // 1. Add a handler for clicking a tooth in the validation SVG (step 2)
  onValidationToothClick(toothNum: number, x: number, y: number) {
    this.openToothPopup(toothNum, x, y);
  }
  
  // Handle mouse enter event on tooth
  onToothMouseEnter(toothNum: number, x: number, y: number) {
    this.showToothInfo(toothNum, { clientX: x, clientY: y } as MouseEvent);
  }
  
  hideToothTimer: any = null;

  // Handle mouse leave event on tooth
  onToothMouseLeave() {
    // Clear any existing timer
    if (this.hideToothTimer) {
      clearTimeout(this.hideToothTimer);
    }
    
    // Set a long delay before hiding the tooltip
    this.hideToothTimer = setTimeout(() => {
      if (!this.keepTooltipVisible) {
        this.hoveredTooth = null;
      }
    }, 1000); // Use a full second to give plenty of time to move to the tooltip
  }

  // Get the color for a specific treatment type
  getTreatmentColor(treatmentValue: string): string {
    // Only color the 'autre' treatment, all standard treatments have no color
    switch(treatmentValue) {
      case 'autre': return '#FFD580'; // Light orange for 'autre' treatment
      default: return 'transparent';
    }
  }

  viewOrdonnanceForTooth(toothNum: number) {
    console.log('[viewOrdonnanceForTooth] Called for toothNum:', toothNum, 'patientId:', this.fichePatient.patientId);
    this.ordonnanceService.getOrdonnanceForTooth(this.fichePatient.patientId, toothNum).subscribe(ordonnances => {
      console.log('[viewOrdonnanceForTooth] ordonnances result:', ordonnances);
      if (ordonnances && ordonnances.length > 0) {
        const ordonnanceToView = ordonnances[0];
        
        console.log('Original ordonnance data:', ordonnanceToView);
        
        // Create a fixed date string for display
        const today = new Date();
        const fixedDateStr = today.getDate() + ' ' + 
                       ['janvier', 'février', 'mars', 'avril', 'mai', 'juin', 'juillet', 'août', 'septembre', 'octobre', 'novembre', 'décembre'][today.getMonth()] + ' ' + 
                       today.getFullYear();
        
        // Try to get the signature from localStorage
        const signatureKey = `signature_${this.fichePatient.patientId}_${toothNum}`;
        const savedSignature = localStorage.getItem(signatureKey);
        
        console.log('Looking for signature with key:', signatureKey);
        console.log('Found signature in localStorage:', !!savedSignature);
        
        // Open the preview dialog with the ordonnance data
        this.dialog.open(OrdonnanceDetailsDialog, {
          data: { 
            ordonnance: { 
              ...ordonnanceToView,
              // Force a valid date object
              date: new Date(),
              // Force a valid date string
              dateStr: fixedDateStr,
              // Use the signature from localStorage if available
              signature: savedSignature || ordonnanceToView.signature
            } 
          },
          height: '500px',
          width: '500px',
        ...
        this.snackBar.open('Aucune ordonnance trouvée pour cette dent.', 'Fermer', { duration: 3000 });
      }
    });
  }
} 