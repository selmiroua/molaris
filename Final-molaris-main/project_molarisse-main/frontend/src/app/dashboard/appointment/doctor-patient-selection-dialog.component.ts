...
export interface DoctorPatientSelectionData {
  selectedDate: Date;
  doctorId: number;
}
...
@Component({
  selector: 'app-doctor-patient-selection-dialog',
  standalone: true,
  imports: [
    CommonModule,
    MatDialogModule,
...
export class DoctorPatientSelectionDialogComponent implements OnInit {
  patients: Patient[] = [];
  ...
  constructor(
    public dialogRef: MatDialogRef<DoctorPatientSelectionDialogComponent>,
    @Inject(MAT_DIALOG_DATA) public data: DoctorPatientSelectionData,
  ...
  loadPatients(): void {
    ...
    // Verify that the doctor ID is provided
    ...
    // Load patients for the specified doctor
    this.userService.getDoctorPatientsById(this.data.doctorId).subscribe({
