...
export interface FichePatient {
  ...
  // Temporary form fields for edit/create dialogs
  priseMedicamentsOption?: string;
  allergyLatex?: string;
  allergyPenicillin?: string;
  allergyAnesthetics?: string;
  allergyIodine?: string;
  allergyMetals?: string;
  allergyOther?: string;
...
export class PatientService {
  ...
  updatePatientFiche(fichePatient: FichePatient): Observable<FichePatient> {
    ...
    console.log('Token available in updatePatientFiche:', !!token);
    ...
    return this.http.post<FichePatient>(`${this.apiUrl}/me/fiche`, fichePatient, { headers }).pipe(
  ...
  // Update patient's medical information by ID using PUT (for doctors/secretaries)
  updateFicheById(patientId: number, fichePatient: FichePatient): Observable<FichePatient> {
    // Get token directly from localStorage
    const token = localStorage.getItem('access_token');
    console.log('Token available in updateFicheById:', !!token);
    ...
    return this.http.put<FichePatient>(`${this.apiUrl}/${patientId}/fiche`, fichePatient, { headers }).pipe(
      tap(response => console.log('Updated fiche by ID:', response)),
      catchError(error => {
        console.error('Error updating fiche by ID:', error);
        return throwError(() => error);
      })
    );
  }
  ...
  // Update a patient's fiche by ID (for doctors/secretaries)
  updateFichePatient(patientId: number, fichePatient: FichePatient): Observable<FichePatient> {
    return this.http.put<FichePatient>(`${this.apiUrl}/${patientId}/fiche`, fichePatient).pipe(
      tap(response => console.log('Updated fiche:', response)),
      catchError(error => {
        console.error('Error updating fiche:', error);
        return throwError(() => error);
